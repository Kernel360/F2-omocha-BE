package org.omocha.core.exception;

import static org.omocha.client.common.code.AuctionCode.*;

import org.omocha.client.common.dto.ResultDto;
import org.omocha.core.exception.auction.AuctionException;
import org.omocha.core.exception.bid.BidException;
import org.omocha.core.exception.chat.ChatException;
import org.omocha.core.exception.image.ImageException;
import org.omocha.core.exception.jwt.JwtTokenException;
import org.omocha.core.exception.member.MemberException;
import org.omocha.core.exception.qna.QnaException;
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ResultDto<Object>> handleHttpMediaTypeNotSupported(
		...
		log.error("errorCode: {}, url: {}, message: {}",
			AuctionCode.UNSUPPORTED_MEDIA_TYPE, request.getRequestURI(), e.getMessage(), e);

		ResultDto<Object> resultDto = ResultDto.res(
			AuctionCode.UNSUPPORTED_MEDIA_TYPE.getStatusCode(),
			AuctionCode.UNSUPPORTED_MEDIA_TYPE.getResultMsg()
		);
		return ResponseEntity
			.status(AuctionCode.UNSUPPORTED_MEDIA_TYPE.getHttpStatus())
			.body(resultDto);
	}
	...
	public ResponseEntity<ResultDto<Object>> handleGeneralException(
		...
		ResultDto<Object> resultDto = ResultDto.res(
			AuctionCode.INTERNAL_SERVER_ERROR.getStatusCode(),
			AuctionCode.INTERNAL_SERVER_ERROR.getResultMsg()
		);
		return ResponseEntity
			.status(AuctionCode.INTERNAL_SERVER_ERROR.getHttpStatus())
			.body(resultDto);
	}
}