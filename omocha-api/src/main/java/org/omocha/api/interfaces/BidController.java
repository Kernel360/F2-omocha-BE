...
import static org.omocha.client.common.code.BidCode.*;

import java.util.List;

import org.omocha.client.auction.application.AuctionService;
import org.omocha.client.bid.application.BidService;
import org.omocha.client.bid.interfaces.request.CreateBidRequest;
import org.omocha.client.bid.interfaces.response.BidResponse;
import org.omocha.client.bid.interfaces.response.CreateBidResponse;
import org.omocha.client.bid.interfaces.response.NowPriceResponse;
...
public class BidController implements BidApi {
	...
	public ResponseEntity<ResultDto<List<BidResponse>>> bidList(
		...
		ResultDto<List<BidResponse>> resultDto = ResultDto.res(
			BidCode.BIDDING_GET_SUCCESS.getStatusCode(),
			BidCode.BIDDING_GET_SUCCESS.getResultMsg(),
			bidList
		);

		return ResponseEntity
			.status(BidCode.BIDDING_GET_SUCCESS.getHttpStatus())
			.body(resultDto);
	}
	...
	public ResponseEntity<ResultDto<CreateBidResponse>> bidAdd(
		...
		ResultDto<CreateBidResponse> resultDto = ResultDto.res(
			BidCode.BIDDING_CREATE_SUCCESS.getStatusCode(),
			BidCode.BIDDING_CREATE_SUCCESS.getResultMsg(),
			createBidResponse
		);

		return ResponseEntity
			.status(BidCode.BIDDING_CREATE_SUCCESS.getHttpStatus())
			.body(resultDto);
	}
	...
	public ResponseEntity<ResultDto<NowPriceResponse>> nowPrice(
		...
		ResultDto<NowPriceResponse> resultDto = ResultDto.res(
			BidCode.NOW_PRICE_GET_SUCCESS.getStatusCode(),
			BidCode.NOW_PRICE_GET_SUCCESS.getResultMsg(),
			nowPriceResponse
		);

		return ResponseEntity
			.status(BidCode.NOW_PRICE_GET_SUCCESS.getHttpStatus())
			.body(resultDto);
	}
}
